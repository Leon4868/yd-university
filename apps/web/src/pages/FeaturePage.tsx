import { ArrowRight, CheckCircle2, CircleDollarSign, Coins, FilePlus2, ShieldAlert } from "lucide-react";
...
export function FeaturePage({ type }: { type: keyof typeof configs }) {
  ...
  return (
    <div className="page-container feature-page">
      ...
      <div className="feature-workspace"><div><h2>USDC → YD</h2><p>页面结构已按 Stitch 设计系统建立，真实链上操作将在合约地址配置完成后启用。</p><div className="placeholder-rows"><span><CheckCircle2 />角色与数据边界已定义</span><span><FilePlus2 />兑换路由待下一阶段接线</span><span><ShieldAlert />危险操作将要求钱包二次确认</span></div></div><button className="button primary">{config.action}<ArrowRight size={18} /></button></div>
