...
export class MockCourseRepository implements CourseRepository {
  private readonly courses: CourseDetail[];

  constructor(courses: CourseDetail[] = demoCourses) {
    this.courses = courses;
  }

  async listPublished(): Promise<CourseSummary[]> {
    return this.publishedCourses().map(toSummary);
  }

  async findPublishedDetailBySlug(slug: string): Promise<CourseDetail | null> {
    return this.publishedCourses().find((course) => course.slug === slug) ?? null;
  }

  private publishedCourses(): CourseDetail[] {
    return this.courses.filter((course) => course.status === "published");
  }
}

function toSummary({ sections: _sections, ...summary }: CourseDetail): CourseSummary {
  return summary;
}
